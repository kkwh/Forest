...
--------------------------------------------------------
  ...
  USING INDEX "PROJECT"."EVENT_PK"  ENABLE;
...
, CONSTRAINT BOOKMARK_PK PRIMARY KEY 
  ...
  USING INDEX 
  (
      CREATE UNIQUE INDEX BOOKMARK_PK ON BOOKMARKS (ID ASC) 
      LOGGING 
      TABLESPACE USERS 
      PCTFREE 10 
      INITRANS 2 
...
BEGIN
  ...
  BEGIN
    IF INSERTING AND :NEW.ID IS NULL THEN
      SELECT BOOKMARKS_SEQ.NEXTVAL INTO :NEW.ID FROM SYS.DUAL;
    END IF;
  END COLUMN_SEQUENCES;
END;
/
