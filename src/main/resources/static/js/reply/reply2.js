    ...
    const makeReplyElements = (data) => {
        ...
        if(reply.userId == 0 & reply.replyPassword !== null) {    
            ...
            <div class="card my-2">
                ...
                <textarea id="replyText_${reply.id}" onclick="toggleReReply(${reply.id})" readonly>${reply.replyText}</textarea>
                <div class="my-2">
        ...
        const showReplies = async (replyId) => {
            ...
            for (let reReply of reReplies) {
                htmlStr += `
                    <div>
                        <span>고정값</span>
                        <span class="fw-bold">${reReply.replyNickname2}</span>
                        <span class="fw-bold">(${reReply.replyIp2})</span>
